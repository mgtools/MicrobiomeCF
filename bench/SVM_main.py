...
def main():
    ...
    os.makedirs('Results/SVM_plots', exist_ok=True)
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(X, y_all)):
        ...
        plot_confusion_matrix(train_conf_matrix, 
                              ...
                              save_path=f'Results/SVM_plots/fold_{fold+1}_train_conf_matrix.png',
                              class_names=['Class 0', 'Class 1'])
        plot_confusion_matrix(val_conf_matrix, 
                              title=f'Validation Confusion Matrix - Fold {fold+1}', 
                              save_path=f'Results/SVM_plots/fold_{fold+1}_val_conf_matrix.png',
                              class_names=['Class 0', 'Class 1'])
        plot_confusion_matrix(test_conf_matrix, 
                              title=f'Test Confusion Matrix - Fold {fold+1}', 
                              save_path=f'Results/SVM_plots/fold_{fold+1}_test_conf_matrix.png',
                              class_names=['Class 0', 'Class 1'])
    ...
    def average_confusion_matrix(metrics_list):
        # Assuming all conf matrices have same shape
    ...
    plot_confusion_matrix(avg_train_cm, 
                          ...
                          save_path='Results/SVM_plots/average_train_conf_matrix.png',
                          class_names=['Class 0', 'Class 1'])

    plot_confusion_matrix(avg_val_cm, 
                          title='Average Validation Confusion Matrix', 
                          save_path='Results/SVM_plots/average_val_conf_matrix.png',
                          class_names=['Class 0', 'Class 1'])

    plot_confusion_matrix(avg_test_cm, 
                          title='Average Test Confusion Matrix', 
                          save_path='Results/SVM_plots/average_test_conf_matrix.png',
                          class_names=['Class 0', 'Class 1'])

    # Save all metrics to a CSV file
    # We'll save one row per fold plus one row for the average
    # Train/Val/Test metrics will be side-by-side for easier comparison
    fold_data = []
    ...
    df_metrics.to_csv('Results/SVM_plots/fold_metrics.csv', index=False)
